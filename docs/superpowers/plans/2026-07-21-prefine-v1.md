# PreFine v1 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the approved single-user PreFine v1 with secure login, strict RMB conversion, cached 12366 tax calendars, personalized tax filtering, email reminders, a responsive React interface, and single-container deployment.
...
- The final image runs one Uvicorn worker as a non-root user and persists `/data/prefine.db`.
...
`test: cover PreFine v1 acceptance flows`
